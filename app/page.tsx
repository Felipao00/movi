...
export default function HomePage() {
    ...
    return (
      <div className="min-h-screen bg-[#FAFAFA] flex flex-col">
        <div className="flex-1 flex flex-col lg:flex-row">
          <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 items-center justify-center relative overflow-hidden">
            <div className="absolute inset-0 opacity-10"><div className="grid grid-cols-8 gap-4 h-full">{Array.from({ length: 64 }).map((_, i) => (<div key={i} className="border border-white/10 rounded-lg" />))}</div></div>
            <div className="relative text-center px-12"><div className="w-32 h-32 flex items-center justify-center mx-auto mb-8"><img src="/logo-branca.png" alt="MOVI" className="w-24 h-24 object-contain" /></div><h1 className="font-signature text-7xl text-white mb-4">MOVI</h1><p className="text-white/60 text-lg font-light max-w-sm mx-auto leading-relaxed">Seu espaço para compartilhar momentos através da fotografia.</p></div>
            <div className="absolute bottom-8 left-0 right-0 text-center"><p className="text-white/30 text-xs">From</p><p className="text-white/50 text-sm font-semibold">Bleth</p></div>
          </div>
          <div className="flex-1 flex items-center justify-center p-6 lg:p-12">
            <div className="w-full max-w-sm">
              <div className="lg:hidden text-center mb-10"><div className="w-20 h-20 flex items-center justify-center mx-auto mb-4"><img src="/logo.png" alt="MOVI" className="w-16 h-16 object-contain" /></div><h1 className="font-signature text-5xl text-gray-900">MOVI</h1><p className="text-gray-500 text-sm mt-2">Seu espaço fotográfico</p></div>
              <div className="hidden lg:block mb-10"><h2 className="text-3xl font-bold text-gray-900 mb-2">Bem-vindo ao MOVI</h2><p className="text-gray-500">Entre ou crie sua conta para começar.</p></div>
              <div className="space-y-3"><Link href="/login" className="flex items-center justify-center w-full py-4 rounded-2xl bg-gray-900 text-white font-medium text-sm hover:bg-gray-800 transition-all active:scale-[0.98]">Entrar na minha conta</Link><Link href="/cadastro" className="flex items-center justify-center w-full py-4 rounded-2xl bg-white border-2 border-gray-200 text-gray-900 font-medium text-sm hover:border-gray-300 hover:bg-gray-50 transition-all active:scale-[0.98]">Criar novo espaço</Link></div>
              <div className="mt-10 grid grid-cols-3 gap-4 text-center"><div><div className="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center mx-auto mb-2"><Camera className="w-5 h-5 text-gray-600" /></div><p className="text-gray-900 text-xs font-medium">Suas fotos</p></div><div><div className="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center mx-auto mb-2"><Compass className="w-5 h-5 text-gray-600" /></div><p className="text-gray-900 text-xs font-medium">Explore</p></div><div><div className="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center mx-auto mb-2"><User className="w-5 h-5 text-gray-600" /></div><p className="text-gray-900 text-xs font-medium">Conecte-se</p></div></div>
              <p className="text-center text-gray-400 text-xs mt-8">Ao entrar, você concorda com nossos termos.</p>
              <div className="lg:hidden text-center mt-8"><p className="text-gray-400 text-xs">From</p><p className="text-gray-500 text-sm font-semibold">Bleth</p></div>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA]">
                ...
                <div key={photo.id} className="break-inside-avoid cursor-pointer" onClick={() => setSelectedPhoto(photo)}>
                  {photo.image_url ? (
                    <div className="rounded-lg overflow-hidden bg-gray-100 relative group">
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100"><div className="flex items-center gap-4 text-white"><span className="flex items-center gap-1 text-sm font-medium"><Heart className="w-5 h-5 fill-white" />{likes[photo.id] || 0}</span></div></div>
                      {followingUsers.has(photo.user_id) && (<div className="absolute top-2 left-2 px-2 py-0.5 rounded-full bg-white/20 backdrop-blur-sm text-white text-[10px] font-medium">Seguindo</div>)}
                    </div>
                  ) : (
                    <div className="rounded-lg bg-gray-900 text-white p-5 min-h-[120px] flex items-center justify-center relative group">
                      <p className="text-sm font-medium text-center leading-relaxed">{photo.title}</p>
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100"><div className="flex items-center gap-4 text-white"><span className="flex items-center gap-1 text-sm font-medium"><Heart className="w-5 h-5 fill-white" />{likes[photo.id] || 0}</span></div></div>
                      {followingUsers.has(photo.user_id) && (<div className="absolute top-2 left-2 px-2 py-0.5 rounded-full bg-white/20 backdrop-blur-sm text-white text-[10px] font-medium">Seguindo</div>)}
                    </div>
                  )}
                </div>
      ...
      <AnimatePresence>
        {showUpload && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-[#FAFAFA] flex flex-col">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 flex-shrink-0"><button onClick={() => { setShowUpload(false); setUploadPreview(null); setUploadTitle(''); }} className="p-2 -ml-2 text-gray-500 hover:text-gray-900"><X className="w-5 h-5" /></button><span className="text-base font-semibold text-gray-900">Nova publicação</span><button onClick={handlePublish} disabled={(!uploadFile && !uploadTitle.trim()) || uploading} className="px-4 py-1.5 rounded-full bg-gray-900 text-white text-sm font-medium disabled:opacity-30 transition-all">{uploading ? '...' : 'Publicar'}</button></div>
            <div className="flex-1 flex flex-col">
              <textarea value={uploadTitle} onChange={(e) => setUploadTitle(e.target.value)} placeholder="O que você quer compartilhar?" className="flex-1 w-full px-5 py-4 bg-transparent text-gray-900 placeholder:text-gray-400 text-lg resize-none focus:outline-none" autoFocus />
              {uploadPreview && (<div className="relative px-4 pb-4"><div className="relative rounded-2xl overflow-hidden"><img src={uploadPreview} alt="Preview" className="w-full max-h-80 object-cover" /><button onClick={() => { setUploadPreview(null); setUploadFile(null); }} className="absolute top-2 right-2 w-8 h-8 rounded-full bg-black/50 text-white flex items-center justify-center"><X className="w-4 h-4" /></button></div></div>)}
            </div>
            <div className="flex items-center gap-2 px-4 py-3 border-t border-gray-200 flex-shrink-0"><label className="p-2.5 rounded-xl bg-gray-100 text-gray-600 hover:bg-gray-200 transition-all cursor-pointer"><ImagePlus className="w-5 h-5" /><input type="file" accept="image/*" onChange={handleFileSelect} className="hidden" /></label>{!uploadPreview && !uploadTitle.trim() && <p className="text-gray-400 text-xs ml-1">Adicione uma foto ou escreva algo</p>}</div>
          </motion.div>
        )}
      </AnimatePresence>
