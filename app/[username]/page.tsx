...
export default function PublicProfilePage() {
  ...
  const loadFollowers = async () => { const { data } = await supabase.from('follows').select('follower_id, profiles:follower_id(id, username, full_name, avatar_url, verified, bio)').eq('following_id', profile.id); setFollowersList(data || []); setShowFollowers(true); };
  const loadFollowing = async () => { const { data } = await supabase.from('follows').select('following_id, profiles:following_id(id, username, full_name, avatar_url, verified)').eq('follower_id', profile.id); setFollowingList(data || []); setShowFollowing(true); };
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA]">
                ...
                <div key={photo.id} className="break-inside-avoid relative group">
                  <div className="cursor-pointer" onClick={() => setSelectedPhoto(photo)}>
                    {photo.image_url ? (
                      <div className="rounded-lg overflow-hidden bg-gray-100">
                        <img src={photo.image_url} alt={photo.title} className="w-full h-auto" loading="lazy" />
                        {photo.pinned && (<div className="absolute top-2 left-2 px-2 py-0.5 rounded-full bg-white/80 backdrop-blur-sm text-gray-900 text-[10px] font-bold flex items-center gap-1"><Pin className="w-3 h-3" /> Fixado</div>)}
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100"><div className="flex items-center gap-3 text-white"><span className="flex items-center gap-1"><Heart className="w-5 h-5 fill-white" />{likes[photo.id] || 0}</span></div></div>
                      </div>
                    ) : (
                      <div className="rounded-lg bg-gray-900 text-white p-4 aspect-square flex items-center justify-center relative">
                        <p className="text-sm font-medium text-center leading-relaxed">{photo.title}</p>
                        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100"><div className="flex items-center gap-3 text-white"><span className="flex items-center gap-1"><Heart className="w-5 h-5 fill-white" />{likes[photo.id] || 0}</span></div></div>
                      </div>
                    )}
                  </div>
                  {isOwner && (
                    <div className="absolute top-2 right-2 z-10">
                      <button onClick={(e) => { e.stopPropagation(); setMenuPhoto(menuPhoto?.id === photo.id ? null : photo); }} className="p-1.5 rounded-full bg-white/80 backdrop-blur-sm text-gray-700 hover:bg-white shadow-sm"><MoreHorizontal className="w-4 h-4" /></button>
                      {menuPhoto?.id === photo.id && (<div className="absolute top-8 right-0 bg-white rounded-xl shadow-xl border border-gray-200 py-1 min-w-[140px] z-20" onClick={(e) => e.stopPropagation()}><button onClick={() => handlePin(photo)} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50"><Pin className="w-4 h-4" /> {photo.pinned ? 'Desafixar' : 'Fixar'}</button><button onClick={() => handleEdit(photo)} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50"><Edit3 className="w-4 h-4" /> Editar</button><button onClick={() => handleDelete(photo)} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-red-600 hover:bg-red-50"><Trash2 className="w-4 h-4" /> Deletar</button></div>)}
                    </div>
                  )}
                </div>
              ))}
      ...
      {/* Modal Verificado */}
      <AnimatePresence>{showVerifiedInfo && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowVerifiedInfo(false)}><motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl" onClick={(e) => e.stopPropagation()}><div className="text-center"><div className="w-20 h-20 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center mx-auto mb-4 shadow-lg shadow-blue-200"><svg className="w-12 h-12" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="11" fill="white" opacity="0.2"/><path d="M8.5 12L11 14.5L15.5 10" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg></div><h2 className="text-xl font-bold text-gray-900 mb-1">Conta Verificada</h2><p className="text-gray-500 text-sm mb-5">Este selo confirma a autenticidade desta conta na MOVI.</p><div className="bg-gray-50 rounded-2xl p-5 text-left space-y-3 mb-4"><div className="flex items-center justify-between"><span className="text-gray-500 text-sm">Usuário</span><span className="text-gray-900 text-sm font-semibold">@{profile.username}</span></div><div className="flex items-center justify-between"><span className="text-gray-500 text-sm">Nome</span><span className="text-gray-900 text-sm font-semibold">{profile.full_name}</span></div><div className="flex items-center justify-between"><span className="text-gray-500 text-sm">Entrou em</span><span className="text-gray-900 text-sm font-semibold">{profile.created_at ? new Date(profile.created_at).toLocaleDateString('pt-BR', { year: 'numeric', month: 'long', day: 'numeric' }) : '—'}</span></div><div className="flex items-center justify-between"><span className="text-gray-500 text-sm">País</span><span className="text-gray-900 text-sm font-semibold">Brasil 🇧🇷</span></div><div className="flex items-center justify-between"><span className="text-gray-500 text-sm">Verificado em</span><span className="text-gray-900 text-sm font-semibold">{profile.verified_at ? new Date(profile.verified_at).toLocaleDateString('pt-BR', { year: 'numeric', month: 'long', day: 'numeric' }) : new Date().toLocaleDateString('pt-BR', { year: 'numeric', month: 'long', day: 'numeric' })}</span></div><div className="flex items-center justify-between"><span className="text-gray-500 text-sm">Tipo</span><span className="text-gray-900 text-sm font-semibold">{profile.username === 'lipe' ? 'Fundador' : 'Criador de conteúdo'}</span></div></div><div className="bg-blue-50 rounded-2xl p-4 text-left mb-4"><div className="flex items-start gap-2"><Shield className="w-4 h-4 text-blue-500 mt-0.5 flex-shrink-0" /><div><p className="text-blue-800 text-xs font-semibold mb-1">Por que esta conta é verificada?</p><p className="text-blue-700 text-xs leading-relaxed">{profile.username === 'lipe' ? 'Felipe é o fundador da MOVI. Esta conta é verificada para garantir que os usuários saibam que estão interagindo com o criador oficial da plataforma.' : 'Esta conta foi verificada pela equipe MOVI.'}</p></div></div></div><button onClick={() => setShowVerifiedInfo(false)} className="w-full py-3 rounded-xl bg-gray-100 text-gray-900 text-sm font-medium hover:bg-gray-200 transition-all">Entendi</button></div></motion.div></motion.div>)}</AnimatePresence>

      {/* Modal Conectados */}
      <AnimatePresence>{showFollowers && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/50 flex items-end justify-center" onClick={() => setShowFollowers(false)}><motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 300 }} className="bg-white rounded-t-3xl w-full max-w-lg max-h-[85vh] flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}><div className="flex justify-center pt-3 pb-1"><div className="w-10 h-1 rounded-full bg-gray-300" /></div><div className="flex items-center justify-between px-5 py-3 border-b border-gray-100"><h3 className="text-lg font-bold text-gray-900">conectados</h3><button onClick={() => setShowFollowers(false)} className="p-2 -mr-2 text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button></div><div className="overflow-y-auto flex-1">{followersList.length === 0 ? (<div className="flex flex-col items-center justify-center py-20 text-gray-400"><User className="w-12 h-12 mb-3 opacity-30" /><p className="text-sm">Nenhum conectado ainda</p></div>) : (followersList.map((f: any) => (<Link key={f.follower_id} href={`/${f.profiles?.username}`} onClick={() => setShowFollowers(false)} className="flex items-center gap-3 px-5 py-3.5 hover:bg-gray-50 transition-colors"><div className="w-11 h-11 rounded-full bg-gray-100 overflow-hidden flex-shrink-0">{f.profiles?.avatar_url ? <img src={f.profiles.avatar_url} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-gray-400 font-bold">{f.profiles?.full_name?.charAt(0) || '?'}</div>}</div><div className="flex-1 min-w-0"><div className="flex items-center gap-1.5"><p className="text-sm font-semibold text-gray-900 truncate">{f.profiles?.full_name}</p>{f.profiles?.verified && <svg className="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="11" fill="#0095F6"/><path d="M8.5 12L11 14.5L15.5 10" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>}</div><p className="text-gray-400 text-xs">@{f.profiles?.username}</p></div>{f.profiles?.bio && <p className="text-gray-500 text-xs max-w-[120px] truncate">{f.profiles.bio}</p>}</Link>)))}</div></motion.div></motion.div>)}</AnimatePresence>

      {/* Modal Seguindo */}
      <AnimatePresence>{showFollowing && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/50 flex items-end justify-center" onClick={() => setShowFollowing(false)}><motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 300 }} className="bg-white rounded-t-3xl w-full max-w-lg max-h-[85vh] flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}><div className="flex justify-center pt-3 pb-1"><div className="w-10 h-1 rounded-full bg-gray-300" /></div><div className="flex items-center justify-between px-5 py-3 border-b border-gray-100"><h3 className="text-lg font-bold text-gray-900">seguindo</h3><button onClick={() => setShowFollowing(false)} className="p-2 -mr-2 text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button></div><div className="overflow-y-auto flex-1">{followingList.length === 0 ? (<div className="flex flex-col items-center justify-center py-20 text-gray-400"><User className="w-12 h-12 mb-3 opacity-30" /><p className="text-sm">Não segue ninguém ainda</p></div>) : (followingList.map((f: any) => (<Link key={f.following_id} href={`/${f.profiles?.username}`} onClick={() => setShowFollowing(false)} className="flex items-center gap-3 px-5 py-3.5 hover:bg-gray-50 transition-colors"><div className="w-11 h-11 rounded-full bg-gray-100 overflow-hidden flex-shrink-0">{f.profiles?.avatar_url ? <img src={f.profiles.avatar_url} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-gray-400 font-bold">{f.profiles?.full_name?.charAt(0) || '?'}</div>}</div><div className="flex-1 min-w-0"><div className="flex items-center gap-1.5"><p className="text-sm font-semibold text-gray-900 truncate">{f.profiles?.full_name}</p>{f.profiles?.verified && <svg className="w-4 h-4 flex-shrink-0" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="11" fill="#0095F6"/><path d="M8.5 12L11 14.5L15.5 10" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>}</div><p className="text-gray-400 text-xs">@{f.profiles?.username}</p></div><div className="px-3 py-1.5 rounded-lg bg-gray-100 text-gray-600 text-xs font-medium">Seguindo</div></Link>)))}</div></motion.div></motion.div>)}</AnimatePresence>
