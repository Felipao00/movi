      ...
      <header className="sticky top-0 z-40 bg-[#FAFAFA]/90 backdrop-blur-xl border-b border-gray-200">
        <Container>
          <div className="flex items-center h-14">
            ...
            <h1 className="text-base font-semibold text-gray-900 ml-2">Seguindo</h1>
          </div>
        </Container>
      </header>
        ...
        <div className="py-4">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-32 text-gray-400">
              ...
              <p className="text-sm">Não segue ninguém ainda</p>
            </div>
          ) : (
            <div className="space-y-1">
              {following.map((f: any) => (
                <Link key={f.following_id} href={`/${f.profiles?.username}`} className="flex items-center gap-4 p-4 rounded-2xl hover:bg-gray-50 transition-colors">
                  ...
                  <span className="px-3 py-1 rounded-lg bg-gray-100 text-gray-600 text-xs font-medium">Seguindo</span>
                </Link>
              ))}
            </div>
          )}
